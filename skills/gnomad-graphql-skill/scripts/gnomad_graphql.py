#!/usr/bin/env python3
"""Compact gnomAD GraphQL client for ChatGPT-imported skills."""
...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    for key in ("max_items", "max_depth", "timeout_sec"):
        ...
        payload[key] = value
    query_text = query.strip() if isinstance(query, str) else Path(query_path).read_text(encoding="utf-8")
    return {
        "query": query_text,
        "variables": variables,
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        response = requests.post(ENDPOINT, json={"query": config["query"], "variables": config["variables"]}, timeout=config["timeout_sec"])
    ...
    if "errors" in data:
        return error("graphql_error", json.dumps(data["errors"])[:500], warnings=[f"raw_output_path={raw_output_path}"] if raw_output_path else [])
