...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    response_format = (_require_str("response_format", payload.get("response_format")) or "auto").lower()
...
def _json_or_xml_output(data: Any, config: dict[str, Any], raw_output_path: str | None) -> dict[str, Any]:
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        ...
        response = requests.get(url, params=_ncbi_common_params(config["params"]), timeout=config["timeout_sec"])
        ...
        if wants_json or text.lstrip().startswith("{") or "json" in content_type:
            ...
            if config["save_raw"]:
                raw_output_path = _save_raw_text(json.dumps(data, indent=2), config["raw_output_path"], "json")
