...
def _summary_output(data: Any, config: dict[str, Any], raw_output_path: str | None) -> dict[str, Any]:
    record_path = config["record_path"]
    path_used, target = (_infer_target(data) if record_path is None else (record_path, _get_by_path(data, record_path)))
    output = {"ok": True, "source": "ncbi-pmc-oa", "record_path": path_used, "raw_output_path": raw_output_path, "warnings": []}
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        response = requests.get(PMC_OA_URL, params=_ncbi_common_params(config["params"]), timeout=config["timeout_sec"])
        response.raise_for_status()
        raw_output_path = _save_raw_text(response.text, config["raw_output_path"], "xml") if config["save_raw"] else None
        root = ET.fromstring(response.text)
        data = {root.tag.split("}", 1)[-1]: _xml_to_simple(root, config["max_items"], config["max_depth"])}
