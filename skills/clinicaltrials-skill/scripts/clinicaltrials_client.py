...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    if action not in set(PATHS) | {"request"}:
        raise ValueError("`action` must be one of: studies, metadata, search_areas, enums, stats_size, field_values, field_sizes, request.")
    ...
    try:
        if config["action"] == "studies":
            ...
            for _ in range(config["max_pages"]):
                ...
                response = session.get(BASE_URL + config["path"], params=params, timeout=config["timeout_sec"])
                ...
                next_page_token = page.get("nextPageToken") if isinstance(page.get("nextPageToken"), str) else None
                if not next_page_token:
                    break
            raw_output_path = _save_raw(pages if len(pages) > 1 else pages[0], config["raw_output_path"]) if config["save_raw"] and pages else None
            ...
            return {
                ...
                "records": _compact(studies[: config["max_items"]], config["max_items"], config["max_depth"]),
                "raw_output_path": raw_output_path,
                "warnings": [],
            }

        response = session.get(BASE_URL + config["path"], params=config["params"], timeout=config["timeout_sec"])
        response.raise_for_status()
        data = response.json()
            ...
            output.update(
                {
                    ...
                    "records": _compact(target[: config["max_items"]], config["max_items"], config["max_depth"]),
                }
            )
        else:
