...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        ...
        if config["response_format"] == "json" or "json" in content_type or text.lstrip().startswith("{"):
            data = response.json()
        ...
        raw_output_path = _save_raw_text(text, config["raw_output_path"], "txt") if config["save_raw"] else None
