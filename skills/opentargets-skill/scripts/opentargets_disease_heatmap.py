...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    if raw_output_path is not None and (not isinstance(raw_output_path, str) or not raw_output_path.strip()):
...
def fetch_page(ensembl_id: str, page_size: int, page_index: int) -> dict[str, Any]:
    response = requests.post(
        ENDPOINT,
        json={"query": QUERY, "variables": {"ensemblId": ensembl_id, "size": page_size, "index": page_index}},
        timeout=60,
    )
...
def sort_datasource_ids(datasource_ids: set[str]) -> list[str]:
    ...
    return sorted(datasource_ids, key=lambda item: (preferred_index.get(item, len(preferred_index)), label_for_datasource(item)))
...
def build_top_datasources(score_map: dict[str, float], limit: int = 3) -> list[dict[str, Any]]:
    ...
    return [{"id": datasource_id, "label": label_for_datasource(datasource_id), "score": score} for datasource_id, score in ranked]
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        for page_index in range(config["max_pages"]):
            ...
            warnings.append(f"Stopped after `max_pages={config['max_pages']}` before exhausting associated disease pages.")
    ...
    columns = [{"id": datasource_id, "label": label_for_datasource(datasource_id)} for datasource_id in ordered_datasource_ids]
    row_preview = [
        {"disease_name": row["disease_name"], "top_datasources": build_top_datasources(row["datasource_scores"])}
