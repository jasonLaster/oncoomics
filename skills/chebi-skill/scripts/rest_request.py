...
def _infer_target(data: Any) -> tuple[str | None, Any]:
    ...
    if isinstance(data, dict):
        ...
        for key in ("collection", "results", "structures", "activities", "molecules", "mechanisms", "records", "items"):
            value = data.get(key)
            if isinstance(value, list):
                return key, value
    return None, data
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        ...
        auto_json = not wants_text and ("json" in content_type or response.text.lstrip().startswith(("{", "[")))
        ...
        if wants_json or auto_json:
            ...
            if config["save_raw"]:
                raw_output_path = _save_raw_output(raw_output, config["raw_output_path"], config["base_url"], "json")
        ...
        raw_output_path = None
        if config["save_raw"]:
            raw_output_path = _save_raw_output(response.text, config["raw_output_path"], config["base_url"], "txt")
