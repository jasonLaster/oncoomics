...
def parse_input(payload: Any) -> Tuple[str, str, int, bool, Optional[str], float]:
...
def resolve_raw_output_path(canonical_variant: str, raw_output_path: Optional[str]) -> Path:
    if raw_output_path:
...
def main() -> int:
    ...
    try:
        input_type, input_variant, max_results, save_raw, raw_output_path, timeout_sec = parse_input(payload)
    ...
    saved_raw_output_path: Optional[str] = None
    ...
    regions = data.get("regions") if isinstance(data, dict) and isinstance(data.get("regions"), list) else []
