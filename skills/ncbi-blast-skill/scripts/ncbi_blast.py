...
def error(code: str, message: str, warnings: Optional[list[str]] = None) -> Dict[str, Any]:
    return {
        "ok": False,
...
def _parse_str(payload: dict[str, Any], key: str) -> Optional[str]:
...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    if not isinstance(result_format_raw, str) or result_format_raw.lower() not in VALID_RESULT_FORMATS:
    ...
    config = {
        ...
        "wait_timeout_sec": _parse_positive_int(payload, "wait_timeout_sec", DEFAULT_WAIT_TIMEOUT_SEC),
...
def make_session(tool: Optional[str], email: Optional[str]) -> requests.Session:
...
def _derive_raw_output_path(rid: str, result_format: str, raw_output_path: Optional[str]) -> Path:
    if raw_output_path:
...
def _extract_json2_payload(response: requests.Response) -> tuple[Any, str]:
    ...
    if content_type.startswith("application/zip") or raw_bytes.startswith(b"PK\x03\x04"):
        try:
            ...
            raise ValueError("BLAST JSON2 response looked like a ZIP archive but could not be opened.") from exc
        ...
        with zip_file:
            ...
            if manifest_members:
                for member_name in manifest_members:
                    if member_name not in zip_file.namelist():
                        raise ValueError(f"BLAST JSON2 archive referenced missing member {member_name!r}.")
                    payload, _ = _load_json_member(zip_file, member_name)
                    if not isinstance(payload, dict):
                        raise ValueError(f"BLAST JSON2 archive member {member_name!r} was not an object.")
            ...
            if not payload_objects:
                if manifest is not None:
                    raise ValueError("BLAST JSON2 archive manifest did not point to any payload JSON files.")
...
def execute(
    ...
    *,
    ...
    if requests is None:
        return error("missing_dependency", f"`requests` is required to run this script: {REQUESTS_IMPORT_ERROR}")
