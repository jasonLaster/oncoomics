...
class NcbiBlastTests(unittest.TestCase):
    ...
    def test_invalid_inputs_return_invalid_input(self) -> None:
        bad_payloads = [
            {"action": "submit", "program": "blastp", "database": "swissprot", "email": "user@example.com"},
            {"action": "status"},
            {"action": "bogus"},
            {
                "action": "fetch",
        ...
        for payload in bad_payloads:
            with self.subTest(payload=payload):
                ...
                stdout = io.StringIO()
                with mock.patch.object(ncbi_blast.sys, "stdin", stdin), mock.patch.object(
                    ncbi_blast.sys, "stdout", stdout
                ):
