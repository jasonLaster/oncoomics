...
def parse_input(payload: Any) -> dict[str, Any]:
    ...
    for name, value in {"max_items": max_items, "max_depth": max_depth, "timeout_sec": timeout_sec}.items():
...
def execute(payload: Any) -> dict[str, Any]:
    ...
    try:
        if config["action"] == "search":
            ...
            response = requests.get(CLINICAL_TABLES_URL, params=params, timeout=config["timeout_sec"])
            response.raise_for_status()
            data = response.json()
            raw_output = json.dumps(data, indent=2)
            raw_output_path = _save(raw_output, config["raw_output_path"]) if config["save_raw"] else None
            if not isinstance(data, list) or len(data) < 4:
                return error("invalid_response", "Clinical Tables response did not match the expected list shape.")
            total = data[0]
            ...
            return {
                ...
                "display_rows": _compact(display_rows[: config["max_items"]], config["max_items"], config["max_depth"]),
