...
Convex maintains independent forward cursors for raw-log ingestion and progress extraction on every CloudWatch stream. A refresh processes a small page/time-bounded slice, commits the cursor after every complete source page, and lets the next refresh resume from that exact point. The bound applies only to one request: there is no absolute backlog ceiling, so repeated refreshes eventually cross an arbitrarily large retained backlog without requiring one serverless invocation to stay alive. CloudWatch source pages are capped at 500 events so a single page cannot expand into an unfinishable mutation chain.

Cursor writes use compare-and-set against the token the worker read. If overlapping refreshes race, the stale worker may still contribute deduplicated events but cannot overwrite a newer opaque token; once a cursor is on protocol v2, a rolling-deploy client that omits the expected token also loses cursor-write authority. A later refresh continues from the winning cursor. Stream discovery durably registers every observed retry attempt and remains retryable after transient discovery failures. Full inventory refreshes use an indexed due-work queue to rotate through four candidates, advancing at most one raw page and one progress page per candidate so total background work is bounded and a huge stream cannot starve smaller ones. Failed candidates receive capped exponential backoff. The work queue lives in Convex rather than the visible job list, so terminal streams remain eligible after their job ages out of the 24-hour UI inventory.

Running streams are always tailed. Terminal streams verify stability after a two-minute grace window; the public `backfillComplete` flag is false for active jobs and does not become true for terminal jobs until every registered stream has a versioned stable cursor observed after that window. If persistence was unavailable, later refresh slices store per-page chromosome maxima and repair the cumulative aggregate before the API responds.
