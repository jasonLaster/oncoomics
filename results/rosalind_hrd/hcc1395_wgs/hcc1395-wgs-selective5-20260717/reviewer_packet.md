...
Run ID: `hcc1395-wgs-selective5-20260717`
...
| --- | --- | --- | --- | --- |
| wgs_pair_validation | passed | Full-source FASTQs: True; read pairs per end: 568040077; BAM validation: passed. | results/phase3_wgs_smoke/phase3_wgs_summary.json |  |
| small_variant_lane | passed | Truth-depth eligible variants: 300; exact PASS matches: 268. | results/phase3_wgs_smoke/phase3_wgs_summary.json | Public-BAM timing runs may skip local variant calling; do not infer HRD score readiness from this alone. |
...
| sbs96_matrix | passed | 265 usable SNV records for SBS96. | results/phase3_wgs_smoke/signature_assignment_summary.json | SBS3 interpretation remains no-call until thresholds and known-answer performance are locked. |
| sv_evidence | passed | SV evidence rows: 2; discordant mapped pairs: 39983763; CHORD statuses: not_assessable_requires_validated_sv_caller_vcf. | results/phase3_wgs_smoke/sv_evidence_summary.json | CHORD and HRDetect need validated SV caller VCF/BEDPE, not metadata-only evidence. |
| sv_caller_readiness | passed | Candidate SV caller rows: 4; discordant mapped pairs in sidecar: 39983763; ready for clinical interpretation: no. | results/clinicalization/sv_caller_readiness_summary.json | Use this as a readiness gate only after it agrees with the current SV evidence summary. |
...
| --- | --- | --- | --- |
| SigProfilerAssignment | input_ready_threshold_met | Classification is deferred for low mutation count; the matrix is a real VCF-derived output, not a proxy. | Promote to ready only after the required production adapter and known-answer validation pass. |
