...
Run ID: `diana-raw-intake-handoff-materialized-20260617`
...
| dinah_handoff_plan | waiting_for_dinah_files | Steps: 8; samplesheet: manifests/diana_raw_inputs.csv; analysis ID: diana_raw_initial; current state: waiting_for_dinah_files. | results/diana_raw_intake/dinah_handoff_plan.json | Planning artifact only; it does not validate files or authorize human-data cloud upload. |
| run_path | ready_to_validate | Plan with `PYTHONPATH=src /usr/bin/python3 -m diana_omics plan:diana-raw-handoff`; validate with `DIANA_RAW_SAMPLESHEET=manifests/diana_raw_inputs.csv DIANA_RAW_REQUIRE_DATA=1 PYTHONPATH=src /usr/bin/python3 -m diana_omics verify:diana-raw`; stage with `DIANA_RAW_SAMPLESHEET=manifests/diana_raw_inputs.csv DIANA_RAW_REQUIRE_DATA=1 PYTHONPATH=src /usr/bin/python3 -m diana_omics stage:diana-raw`. | results/diana_raw_intake/input_contract.json | Passing intake validation still does not produce an HRD score. |

## HRD Adapter Status
| adapter | state | blocker | next_action |
| --- | --- | --- | --- |
| Raw file intake | blocked_until_files | Actual Diana BAM/FASTQ/CRAM paths have not passed strict intake validation. | Run plan:diana-raw-handoff, copy the template to manifests/diana_raw_inputs.csv, fill actual paths and metadata, then run verify:diana-raw with DIANA_RAW_REQUIRE_DATA=1. |
