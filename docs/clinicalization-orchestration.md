...
The immediate target is now reportable-range/QC threshold locking machinery while the known-answer benchmark approval packet waits for owner review. Asset acquisition planning, checksum capture policy, and approval-packet summaries are defined locally and keep raw upload, execution, and clinical use disabled until explicit approval plus source-published or verified local checksums. Do not start those benchmark runs without explicit approval.
