...
The immediate target is now approved-asset acquisition planning for known-answer benchmarks. Checksum capture policy is defined locally and keeps execution disabled until source-published or verified local checksums are populated after explicit approval. Do not start those benchmark runs without explicit approval.
