...
The immediate target is now local manifest validation for benchmark inputs/truth assets: `benchmark:known-answer --fixture <fixture_id> --dry-run` can materialize expected output paths and cache keys without fetching data or running WGS. Do not start those benchmark runs without explicit approval.
