...
The immediate target is now the known-answer benchmark approval packet: exact source URLs, access terms, expected checksums, and cost/transfer notes. Asset acquisition planning and checksum capture policy are defined locally and keep raw upload, execution, and clinical use disabled until explicit approval plus source-published or verified local checksums. Do not start those benchmark runs without explicit approval.
