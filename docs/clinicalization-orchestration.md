...
The immediate target is now checksum and reference-build compatibility checks for the known-answer benchmark manifests. The input/truth asset manifests are metadata-only and keep benchmark execution disabled until explicit approval. Do not start those benchmark runs without explicit approval.
