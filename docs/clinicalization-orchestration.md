...
- Add a concrete expanded sample pull plan for the first 10 known-answer raw/truth targets. Started locally with `manifests/known_answer_sample_pull_plan.csv` and `verify:known-answer-sample-pull-plan`.
- Add target-by-target public-finding confirmation status. Started locally with `manifests/known_answer_public_finding_checks.csv`, `results/clinicalization/known_answer_public_finding_confirmation.md`, and `verify:known-answer-public-findings`.
