...
The immediate target is now source checksum capture for the known-answer benchmark assets. Reference-build compatibility is checked locally, but all checksums remain pending because the manifests are metadata-only and benchmark execution is disabled until explicit approval. Do not start those benchmark runs without explicit approval.
