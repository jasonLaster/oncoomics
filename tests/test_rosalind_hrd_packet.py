...
class RosalindHrdPacketTest(unittest.TestCase):
    ...
    def test_diana_wgs_packet_rejects_loose_summary_forbidden_tokens(self):
        for field, value, message in (
            (
                "dataset",
                True,
                "Diana WGS forbidden-token dataset must be a non-empty unpadded single-line string",
            ),
            (
                "pair",
                " SENSITIVE-PAIR-ID",
                "Diana WGS forbidden-token pair must be a non-empty unpadded single-line string",
            ),
            (
                "input",
                None,
                "Diana WGS forbidden-token input summary is missing or malformed",
            ),
        ):
            with self.subTest(field=field), tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as artifacts:
                ...
                if field == "input":
                    worker_summary["input"] = value
                else:
                    worker_summary["input"][field] = value
