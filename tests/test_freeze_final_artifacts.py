...
class FreezeFinalArtifactsTests(unittest.TestCase):
    ...
    def test_sha256_rejects_symlinked_hash_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            root = Path(value)
            real_input = root / "real-final-receipt.json"
            linked_input = root / "final-receipt-link.json"
            real_input.write_text("{}\n", encoding="utf-8")
            linked_input.symlink_to(real_input)

            real_parent = root / "real-inputs"
            real_parent.mkdir()
            (real_parent / "final-receipt.json").write_text("{}\n", encoding="utf-8")
            linked_parent = root / "linked-inputs"
            linked_parent.symlink_to(real_parent, target_is_directory=True)
            ...
            cases = (
                (
                    linked_input,
                    "final-receipt-link.json SHA-256 input must be a real file",
                ),
                (
                    linked_parent / "final-receipt.json",
                    "final-receipt.json SHA-256 input parent must not be a symlink",
                ),
            )
    ...
    def test_sha256_rejects_hash_input_that_changes_during_read(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            ...
            replacement = root / "mutated-final-receipt.json"
    ...
    def test_sha256_rejects_same_byte_leaf_replacement(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            root = Path(value)
            receipt = root / "final-receipt.json"
            replacement = root / "replacement-final-receipt.json"
            receipt.write_text('{"stable": true}\n', encoding="utf-8")
    ...
    def test_sha256_rejects_symlink_swap_after_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as value:
            root = Path(value)
            receipt = root / "final-receipt.json"
            target = root / "redirected-final-receipt.json"
            receipt.write_text('{"stable": true}\n', encoding="utf-8")
